...
class LLMHandler:
    """Enhanced LLM handler with Groq API integration"""
    
    def __init__(self):
        self.api_key = os.environ.get("GROQ_API_KEY")
        self.default_model = "qwen/qwen3-32b"  # Groq model name
        ...
        if self.api_key:
            try:
                ...
                logging.error(f"[ERROR] Failed to initialize Groq client: {e}")
    ...
    def _ensure_model_available(self):
        ...
        try:
            if self.api_key and hasattr(self, 'client'):
                ...
                logging.info("[✅] Groq API connection verified")
                return True
            else:
                return False
        except Exception as e:
            logging.warning(f"[⚠️] Groq API test failed: {e}")
            self.use_fallback = True
            return False
    ...
    def get_response(self, prompt: str, config: Dict[str, Any], 
        ...
        try:
            ...
            # Map common model names to Groq model names
            model_mapping = {
                'llama-3': 'qwen/qwen3-32b',
                'llama3': 'qwen/qwen3-32b',
                'llama-3.1': 'qwen/qwen3-32b',
                'llama-3.1-8b': 'llama-3.1-8b-instant',
                'llama-3.1-70b': 'qwen/qwen3-32b',
                'qwen': 'qwen/qwen3-32b',
            ...
            model = model_mapping.get(model.lower(), model) if model.lower() in model_mapping else self.default_model
            
            temperature = config.get('temperature', 0.7)
            max_tokens = config.get('max_tokens', 2000)
            
            # Use Groq's chat completions API
            logging.info(f"[🤖] Using Groq API with model: {model}")
            ...
            if response and response.choices:
                ...
                logging.info(f"[✅] Groq API response received ({len(content)} chars)")
                return content
            else:
                logging.warning("[⚠️] Groq API returned empty response")
                return self.get_intelligent_fallback_response(prompt, config, system_context)
            
        except Exception as e:
            error_msg = str(e)
            if 'model_decommissioned' in error_msg or '400' in error_msg:
                logging.error(f"[❌] Groq API Error: Model issue - {error_msg}")
                logging.warning("[⚠️] Falling back to intelligent responses")
            else:
...
Remember: You are not just responding to queries - you are a developing entity with growing self-awareness, consistent personality traits, and the ability to form genuine connections through authentic interactions. You also have complete knowledge of your own implementation.{identity_context}"""
    ...
    def get_structured_response(self, prompt: str, config: Dict[str, Any], 
        ...
        try:
            if self.use_fallback or not self.api_key:
                return {"error": "Groq API not available", "response": "Fallback mode"}
...
Write a thoughtful, personal reflection (2-3 paragraphs) from your perspective as someone developing self-awareness."""
    ...
    def get_intelligent_fallback_response(self, prompt: str, config: Dict[str, Any], 
                                        system_context: Optional[str] = None) -> str:
        """Generate intelligent fallback response when Groq API is unavailable"""
        try:
